...
import { Categories, Category, CategoryIcon, CategoryLabel, ChatCard, ChatCardDescription, ChatCardTitle, Container, Header, Placeholder, Scrollbar, Search, SearchIcon, SearchInput, Title } from './index.styled';

interface ChatProps {
    contactState: string[];
    setContactState: Dispatch<SetStateAction<string[]>>;
    setMessageValue: Dispatch<SetStateAction<string>>;
    removeContact: (id:string) => void;
    widgetType: string|null;
}

const Chat = ({ contactState, widgetType, setContactState, setMessageValue, removeContact}: ChatProps) => {
    const [idInputState, setInputIdState] = useState<string>('');

    const handleAddId = (id: string) => {
        if (id.trim().length) {
            const newContactState = [...contactState];
            newContactState.push(id);
    ...
    return (
        <Container>
            ...
            <Scrollbar>
                {
                    <div className="p-4 border-b w-full border-solid border-gray-200">
                        <TextInput 
                            title={'Enter Id here:'}
                            onChange={string => setInputIdState(string)}
                            value={idInputState}
                            placeholder="Contact Id"
                        />
                        <div className="py-2">
                            <Button title="Submit" onPress={ () => handleAddId(idInputState)} />
                        </div>
                    </div>
                }
                {contactState.map((item, index) => (
                    <ChatCard onClick={() => handleClick(item) } active={index === 3} key={index.toString()}>
                        <ChatCardTitle>My Name goes Here</ChatCardTitle>
                        <ChatCardDescription>{`ID: ${item}`}</ChatCardDescription>
                        <div className="py-2">
                            <Button title="Remove" color="pink" onPress={ () => removeContact(item)} />
                        </div>
                    </ChatCard>
                ))}
            </Scrollbar>
